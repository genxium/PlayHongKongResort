...
public class Notification extends AbstractActivityMessage {
    ...
    public Notification(JSONObject notificationJson) {
        ...
        if (notificationJson.containsKey(COMMENT_ID))
            m_commentId = Converter.toInteger(notificationJson.get(COMMENT_ID));
        if (notificationJson.containsKey(ASSESSMENT_ID))
            m_assessmentId = Converter.toInteger(notificationJson.get(ASSESSMENT_ID));
